...
const universities = [];
graduates.forEach((unis) => {
  return universities.push(`${unis.university}`);
});
...
const unisWithUni = graduates.filter((name) => {
  return name.university.includes('University');
});
...
zooAnimals.map((lowerName) => {
  return lowCaseAnimalNames.push(`${lowerName.animal_name.toLowerCase()}`);
});
